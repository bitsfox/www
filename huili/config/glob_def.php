...
$SIGN_DEF["bodyf_10_4"]	=	"<div class='form-group'><label>Password</label><div class='form-suffix'><span class='fa fa-lock fa-lg picto'></span><input id='password' type='password' class='form-control last' name='password' value='' placeholder='确认密码' required autocomplete='new-password' pattern='(?=.*[A-Z])(?=.*[a-z])(?=.*[!@#$%^&*()+=?_-])(?=.*[0-9]).{8,32}' title='密码必须包含大小写字母、数字以及特殊符号例如：(!@$&*)' />";
$SIGN_DEF["bodyf_10_5"]	=	"</div><div class='help-block'>密码必须是8-32位字符，且字符中必须出现大小写字母(e.g. Aa), 数字(e.g. 1234), 特殊符号(e.g. !$&*)</div></div>";
$SIGN_DEF["bodyf_10_6"] =	"<div class='form-group'><div class='checkbox'><label><input type='checkbox' id='trusted' name='trusted' checked data-ninja-checkbox> Trust this device <span class='helper' data-toggle='tooltip' data-placement='top' title='Trusting a device means that you will be able to log into the Greenback service without performing any additional verification beyond username and password. If you do not trust a device then you will required to verify your identity by entering a one-time verification code that will be sent the email address linked to your account. You will enter this verification code on the challenge screen that you will be presented with after entering a correct username password combination. '>?</span></label></div></div>";
$SIGN_DEF["bodyf_10_7"]	=	"<div class='form-group form-delimited'><p class='intro'>By signing up you agree to our <a href='/terms' target='_blank'>Terms and Conditions</a> and <a href='/privacy' target='_blank'>Privacy Policy</a>. </p></div>";
$SIGN_DEF["bodyf_10_8"]	=	"<div class='form-group form-delimited'><button type='submit' class='btn btn-primary btn-fat btn-block'>Sign Up</button></div><div class='form-group form-out'><p>Already have an account? <a href='/auth/signin' class='btn-text'>Sign In</a></p></div>";
