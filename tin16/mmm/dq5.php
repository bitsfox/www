<?php
echo "<html><head>
<meta http-equiv='Content-Type' content='text/html; charset=utf-8' />
<link href='./main.css' rel='stylesheet' type='text/css' />";
echo "<style type='text/css'>
body{ text-align:center} 
#divcenter{margin:0 auto;width:800px}</style>
</head><body>";
echo "<br><center><font size=6 color=red>勇者斗恶龙5攻略</font></center><br><br>";
echo "<table align=center width=70% border=1 cellspacing=0 cellpadding=1><tr>";
echo "<td align=center><a href='./walk.php' target=_blank><font size=4>攻略流程</font></a></td>";
echo "<td align=center><a href='../../../counter.php' target=_blank><font size=4>待添加</font></a></td>";
echo "<td align=center><a href='./.php' target=_blank><font size=4>待添加</font></a></td>";
echo "<td align=center><a href='./.php' target=_blank><font size=4>待添加</font></a></td>";
echo "</tr></table>";
echo "</body></html>";
?>
<?php
include_once("/var/www/counter.php");
#upcounter(__FILE__);
?>
<?php
include_once("/var/www/counter.php");
upcounter(__FILE__)
?>
